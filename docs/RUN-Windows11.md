...
On first start the app has **no project folders**. Create a project from the top-left menu, then **+ Add folder…** and paste an absolute Windows path (for example `D:\Notes\html`). The left pane shows that folder tree (HTML, Markdown, PDF, Word, text, CSV, Excel, PowerPoint, RTF, OpenDocument, EPUB, and notebooks). Click a file to open it in the larger right pane.
