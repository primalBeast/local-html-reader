...
DEFAULT_PORT = 8766
APP_NAME = "LocalHtmlReader"
# Explorer-friendly folder in OneDrive for every file this app writes.
WINDOWS_ONEDRIVE_FOLDER = "Local HTML Reader"

logger = logging.getLogger("lhr")
...
def _looks_populated(path: Path) -> bool:
    return (path / "settings.json").is_file()


def migrate_windows_appdata_to_onedrive(dest: Path) -> None:
    """Copy AppData\\LocalHtmlReader into OneDrive, then remove the old copy."""
    ...
    logger.info("Moving Local HTML Reader data from %s to %s", legacy, dest)
    shutil.copytree(
        legacy,
        dest,
        dirs_exist_ok=True,
    ...
    try:
        ...
        logger.warning(
            "Could not fully remove %s after move (%s); leftover files are safe to delete",
            legacy,
            exc,
        )
...
def prepare_data_dir(dest: Path) -> None:
    ...
    if platform.system() != "Windows":
        return
    if os.environ.get("LHR_DATA_DIR"):
        return
...
def default_data_dir() -> Path:
    env = os.environ.get("LHR_DATA_DIR")
    if env:
    ...
    return Path(xdg) / "local-html-reader"
...
class AppConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if os.environ.get("LHR_DEV_CORS") == "1":
            self.dev_cors = True
        if self.reload:
            self.dev_cors = True
